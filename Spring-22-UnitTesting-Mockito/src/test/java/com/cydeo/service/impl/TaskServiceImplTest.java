...
class TaskServiceImplTest {
    ...
    void findById_test(long id) {
        ...
        taskDTOMock.setId(id);

        when(taskRepository.findById(anyLong())).thenReturn(Optional.of(taskMock));
//        when(taskRepository.findById(id)).thenReturn(Optional.of(taskMock));
        when(taskMapper.convertToDTO(taskMock)).thenReturn(taskDTOMock);

        TaskDTO taskDTO = taskService.findById(id);

//        verify(taskRepository).findById(anyLong());
        verify(taskRepository).findById(id);
        // reject negative id values
    ...
    void findById_bdd_test() {
        ...
        when(taskMapper.convertToDTO(taskMock)).thenReturn(new TaskDTO());

        given(taskRepository.findById(anyLong())).willReturn(Optional.of(taskMock));
        given(taskMapper.convertToDTO(taskMock)).willReturn(new TaskDTO());

        TaskDTO taskDTO = taskService.findById(anyLong());

        then(taskRepository).should().findById(anyLong());
        then(taskMapper).should().convertToDTO(any(Task.class));
